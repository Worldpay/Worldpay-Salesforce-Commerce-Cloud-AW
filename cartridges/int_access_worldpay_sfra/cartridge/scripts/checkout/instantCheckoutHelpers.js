...
function beginCheckout(req) {
    ...
    if (!currentBasket) {
        return {
            error: true,
            errorDetail: Resource.msg('instant.checkout.error', 'cart', null)
        };
    }
...
function fillPaymentFormFromBasket(currentBasket, paymentInstrument, cvv) {
    ...
    var token = paymentInstrument.raw.creditCardToken;
    if (paymentInstrument.raw.custom && paymentInstrument.raw.custom.awpCCTokenData) {
        token = paymentInstrument.raw.custom.awpCCTokenData;
    }
    if (Object.prototype.hasOwnProperty.call(paymentInstrument.raw.custom, 'csdkTokenId')) {
        paymentData.paymentInformation.tokenID.value = paymentInstrument.raw.custom.csdkTokenId;
    }
    if (Object.prototype.hasOwnProperty.call(paymentInstrument.raw.custom, 'csdkTokenExp')) {
        paymentData.paymentInformation.tokenExp.value = paymentInstrument.raw.custom.csdkTokenExp;
    }
    // below are duplicate carriers of the token href, to be handled in the refactor flow to align to a single attribute
    paymentData.paymentInformation.creditCardToken = token;
    paymentData.paymentInformation.creditCardTokenData = token;
    paymentData.paymentInformation.tokenPI.value = token;

...
server.post('SubmitOrder', server.middleware.https, function (req, res, next) {
    ...
    if (!isCVVDisabled && !cvv) {
        res.json({
            error: true,
            missingCVV: true,
            cvvErrorMessage: Resource.msg('error.instant.checkout.missing.cvv', 'cart', null)
        });
        this.emit('route:Complete', req, res);
        return;
    }
    if (!isCVVDisabled && !InstantCOHelper.isValidCVV(cvv)) {
        res.json({
            error: true,
            invalidCVV: true,
            cvvErrorMessage: Resource.msg('error.instant.checkout.invalid.cvv', 'cart', null)
        });
        this.emit('route:Complete', req, res);
        return;
    }
    ...
    if (shippingAddressID && billingAddressID && shippingMethodID && savedPaymentID) {
        try {
            ...
            Logger.getLogger('worldpay').error('Exception during instant checkout order ' + e.message);
            res.json({
                error: true,
                errorMessage: Resource.msg('error.cannot.select.shipping.method', 'cart', null)
            });

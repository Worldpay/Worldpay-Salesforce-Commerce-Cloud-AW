...
module.exports = {
    startInstantPurchase: function () {
        $(document).on('click', '#instant-pay-now', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.error && data.redirectUrl) {
                        ...
                        $('.instant-pay-modal').empty().html(data.instantCheckoutContent);
                        $('.instant-pay-modal #instant-checkout').modal('show');
                        $('.instant-pay-modal').on('shown.bs.modal', function () {
    ...
    eventListener3DSInstant: function () {
        window.addEventListener('message', function (event) {
            if (instantCheckout3ds2) {
                var data = JSON.parse(event.data);
                var dataSessionId = data.SessionId;
                var challengeURL = $('#authenticate3dsURL').val();
                $.ajax({
                    url: challengeURL,
    ...
    completeCheckout: function () {
        $(document).on('click', '#complete-checkout', function (e) {
            ...
            instantCheckout3ds2 = $('#instant-checkout #card-iframe') ? $('#instant-checkout #card-iframe').data('isinstantcheckout') : false;
            var postData = {
